Type TestAssert Extends TTest
	Function assertEqualsAud(expected:TAudience, actual:TAudience, message:String = Null)	
		...
		assertEqualsF(expected.Children, actual.Children, 0.00005, message + " [-> Children]")
		assertEqualsF(expected.Teenagers, actual.Teenagers, 0.00005, message + " [-> Teenagers]")
		assertEqualsF(expected.HouseWives, actual.HouseWives, 0.00005, message + " [-> HouseWives]")
		assertEqualsF(expected.Employees, actual.Employees, 0.00005, message + " [-> Employees]")
		assertEqualsF(expected.Unemployed, actual.Unemployed, 0.00005, message + " [-> Unemployed]")
		assertEqualsF(expected.Manager, actual.Manager, 0.00005, message + " [-> Manager]")
		assertEqualsF(expected.Pensioners, actual.Pensioners, 0.00005, message + " [-> Pensioners]")
		assertEqualsF(expected.Women, actual.Women, 0.00005, message + " [-> Women]")
		assertEqualsF(expected.Men, actual.Men, 0.00005, message + " [-> Men]")
	End Function
