...
'		If audience > 0
	...
	Function GetPotentialAudiencePercentage_TimeMod:TAudience(time:Long = -1)
		...
		Select GetWorldTime().GetDayHour(time)
			...
			'                                  |    |    |    |    |    |  manager
			'                                  |    |    |    |    |    |    |  pensioners
			Case 0  result = New TAudience.Set(-1,   2,   6,  16,  11,  21,  19,  23)
			Case 1  result = New TAudience.Set(-1, 0.5,   4,   7,   7,  15,   9,  13)
			Case 2  result = New TAudience.Set(-1, 0.2,   1,   4,   4,  10,   3,   8)
			Case 3  result = New TAudience.Set(-1, 0.1,   1,   3,   3,   7,   2,   5)
			Case 4  result = New TAudience.Set(-1, 0.1, 0.5,   2,   3,   4,   1,   3)
			Case 5  result = New TAudience.Set(-1, 0.2,   1,   2,   4,   3,   3,   3)
			Case 6  result = New TAudience.Set(-1,   1,   2,   3,   6,   3,   5,   4)
			Case 7  result = New TAudience.Set(-1,   4,   7,   6,   8,   4,   8,   5)
			Case 8  result = New TAudience.Set(-1,   5,   2,   8,   5,   7,  11,   8)
			Case 9  result = New TAudience.Set(-1,   6,   2,   9,   3,  12,   7,  10)
			Case 10 result = New TAudience.Set(-1,   5,   2,   9,   3,  14,   3,  11)
			Case 11 result = New TAudience.Set(-1,   5,   3,   9,   4,  15,   4,  12)
			Case 12 result = New TAudience.Set(-1,   7,   9,  11,   8,  15,   8,  13)
			Case 13 result = New TAudience.Set(-1,   8,  12,  14,   7,  24,   6,  19)
			Case 14 result = New TAudience.Set(-1,  10,  13,  15,   6,  31,   2,  21)
			Case 15 result = New TAudience.Set(-1,  11,  14,  16,   6,  29,   2,  22)
			Case 16 result = New TAudience.Set(-1,  11,  15,  21,   6,  35,   2,  24)
			Case 17 result = New TAudience.Set(-1,  12,  16,  22,  11,  36,   3,  25)
			Case 18 result = New TAudience.Set(-1,  16,  21,  24,  18,  39,   5,  34)
			Case 19 result = New TAudience.Set(-1,  24,  33,  28,  28,  46,  12,  49)
			Case 20 result = New TAudience.Set(-1,  16,  36,  33,  37,  56,  23,  60)
			Case 21 result = New TAudience.Set(-1,  11,  32,  31,  36,  53,  25,  62)
			Case 22 result = New TAudience.Set(-1,   5,  23,  30,  30,  46,  35,  49)
			Case 23 result = New TAudience.Set(-1,   3,  14,  24,  20,  34,  33,  35)
		EndSelect
		'MAX is 62% -> modifiers should pay attention
		'-> limit mods to 100/62.0

		'convert to 0-1.0 percentage
		Return result.Multiply(0.01)
	End Function
	...
	Function GetPotentialAudiencePercentage_WeatherMod:TAudience(time:Long = -1)
		...
		Return New TAudience.Set(weatherMod, weatherMod)
	End Function


	Function GetPotentialAudienceModifier:TAudience(time:Long = -1)
		Local modifier:TAudience = New TAudience.Set(1, 1)
...
Type TBroadcastFeedback
	...
	Global audience_AdultOnlyMask:TAudience = New TAudience.Set(-1,  0, 1, 1, 1, 1, 1, 1)
	...
	Method CalculateAudienceInterestForAllowed(attr:TAudienceAttraction, maxAudienceMod:TAudience, plausibility:TAudience, minAttraction:Float)
		...
		If attr.Quality > 0.8
			...
			If highestAllowedAdult And highestAllowedAdult.Value >= 0.9 And attr.Quality > 0.8 Then
				allowedAll = New TAudience.Set(1, 1)
	...
	Method CalculateAudienceInterest(bc:TBroadcast, attr:TAudienceAttraction)
		...
		If (hour >= 0 And hour <= 1)
			CalculateAudienceInterestForAllowed(attr, maxAudienceMod, New TAudience.Set(-1,  0, 1, 2, 1, 2, 1, 2), 0.10)
		ElseIf (hour >= 2 And hour <= 5)
			CalculateAudienceInterestForAllowed(attr, maxAudienceMod, New TAudience.Set(-1,  0, 0, 1, 0, 2, 0, 2), 0.225)
		ElseIf (hour = 6)
			CalculateAudienceInterestForAllowed(attr, maxAudienceMod, New TAudience.Set(-1,  1, 1, 1, 1, 0, 1, 1), 0.20)
		ElseIf (hour >= 7 And hour <= 8)
			CalculateAudienceInterestForAllowed(attr, maxAudienceMod, New TAudience.Set(-1,  1, 1, 1, 1, 0, 1, 1), 0.125)
		ElseIf (hour >= 9 And hour <= 11)
			CalculateAudienceInterestForAllowed(attr, maxAudienceMod, New TAudience.Set(-1,  0, 0, 2, 0, 1, 0, 2), 0.125)
		ElseIf (hour >= 13 And hour <= 16)
			CalculateAudienceInterestForAllowed(attr, maxAudienceMod, New TAudience.Set(-1,  2, 2, 2, 0, 2, 0, 2), 0.05)
		ElseIf (hour >= 22 And hour <= 23)
			CalculateAudienceInterestForAllowed(attr, maxAudienceMod, New TAudience.Set(-1,  0, 1, 2, 2, 2, 2, 2), 0.05)
		Else
			CalculateAudienceInterestForAllowed(attr, maxAudienceMod, New TAudience.Set(-1,  1, 2, 2, 2, 2, 1, 2), 0.05)
		EndIf
	End Method
...
Type TAudienceMarketCalculation
	...
	Method ComputeAudience(time:Long = -1)
		...
		For Local channelID:Int = 1 To 4
			...
			Local effectiveAttraction:TAudience = attraction.FinalAttraction.Copy().Multiply(competitionAttractionModifier)
	...
	Method GetCompetitionAttractionModifier:TAudience()
		...
		For Local attraction:TAudienceAttraction = EachIn audienceAttractions
			If attrSum = Null
				attrSum = attraction.FinalAttraction.Copy()
			Else
				attrSum.Add(attraction.FinalAttraction)
			EndIf

			If attrRange = Null
				attrRange = attraction.FinalAttraction.Copy()
			Else
