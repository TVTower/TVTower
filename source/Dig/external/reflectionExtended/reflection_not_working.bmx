REM
	' ===== EXTENDED REFLECTION =====
	THIS CODE IS FROM
	http://www.blitzmax.com/Community/posts.php?topic=84918#959795

	externalized to make it work without modifying the modules folder
...
Function _CallFunction:Object( funcp:Byte Ptr, rettype:TTypeId, args:Object[], argtypes:TTypeId[])
	Local q:Int[10], sp:Byte Ptr = q
	If rettype = LongTypeId sp :+ 8
...
Type TField Extends TMember
	...
	Rem
	bbdoc Get the pointer to a field
...
Type TMethod Extends TMember
	...
	Method Invoke:Object( obj:Object,args:Object[] )
		If _index<65536
			Return _CallMethod( bbRefMethodPtr( obj,_index ),_typeId,obj,args,_typeId._argTypes )
		EndIf
...
EndRem
		...
		While p[0]
			...
			Select p[0]
				...
				Case 6	'method
					Local tt:TTypeId = TypeIdForTag(ty)
					If tt Then
						_methods.AddLast New TMethod.Init( id, tt, meta, Self, p[3], BuildArgArray(ty))
					EndIf

				Case 7	' function
					Local tt:TTypeId = TypeIdForTag(ty)
					If tt Then
						_functions.AddLast New TFunction.Init(id, tt, meta, Self, p[3], BuildArgArray(ty))
					EndIf
			EndSelect
			p:+4
		Wend
	...
	Function BuildArgArray:TTypeID[](typeString:string)
		Local t:string[] = typeString.Split( ")" )
		...
		If t[0].length>1
			...
			While i<q.length
				...
				End Select
			Wend
			If b<q.length then args.AddLast q[b..q.length]
