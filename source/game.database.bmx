...
Type TDatabaseLoader
	...
	Function ConvertOldScriptExpression:String(expression:string, oldNewMapping:TStringMap, scriptExpressionConverterSB:TStringBuilder, changedSomething:Int Var)
		...
		If expression.Find("%TEAM")>=0
			...
			expression = expression.Replace("%TEAM1STARSHORT%", "${.self:~qsportteam~q:0:~qmembers~q:-1:~qlastname~q}")
			expression = expression.Replace("%TEAM2STARSHORT%", "${.self:~qsportteam~q:1:~qmembers~q:-1:~qlastname~q}")
			expression = expression.Replace("%TEAM1STAR%", "${.self:~qsportteam~q:0:~qmembers~q:-1:~qfullname~q}")
			expression = expression.Replace("%TEAM2STAR%", "${.self:~qsportteam~q:1:~qmembers~q:-1:~qfullname~q}")
			expression = expression.Replace("%TEAM1KEEPERSHORT%", "${.self:~qsportteam~q:0:~qmembers~q:0:~qlastname~q}")
			expression = expression.Replace("%TEAM2KEEPERSHORT%", "${.self:~qsportteam~q:1:~qmembers~q:0:~qlastname~q}")
			expression = expression.Replace("%TEAM1KEEPER%", "${.self:~qsportteam~q:0:~qmembers~q:0:~qfullname~q}")
			expression = expression.Replace("%TEAM2KEEPER%", "${.self:~qsportteam~q:1:~qmembers~q:0:~qfullname~q}")
