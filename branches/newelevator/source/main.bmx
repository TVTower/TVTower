...
Type TFloorRoute
	...
	Field call:Int
	Field who:TFigures
	Field initialFrom:int 'Kann wieder raus... nur für's Debuggen gut
...
REM
	...
	End Function
ENDREM
	Function Create:TFloorRoute(floornumber:Int, call:Int=0, who:TFigures=null)
		Local floorRoute:TFloorRoute = New TFloorRoute
		floorRoute.floornumber = floornumber
		floorRoute.call = call
		floorRoute.who = who		
		floorRoute.initialFrom = Building.Elevator.CurrentFloor		
		Return floorRoute
	End Function
		
	Method GetCallIndex:Int()
		local index:int = 0
		For Local route:TFloorRoute = EachIn Building.Elevator.FloorRouteList
			if route = self Then Return index
			index = index + 1
		Next		
	End Method
	
	Method GetIntendedDirection:Int()
		If call = 1
			local targetFloor:int = who.getFloor(who.target)
			If (floornumber = targetFloor) Then Return 0			
			If (floornumber < targetFloor) Then Return 1 Else Return -1
		Else
			Return GetRelativeDirection()
		Endif	
	End Method
	
	Method GetRelativeDirection:Int()
		If (Building.Elevator.CurrentFloor = floornumber) Then Return 0
		If (Building.Elevator.CurrentFloor < floornumber) Then Return 1 Else Return -1		
	End Method	
	
	Method CorrectDirection:Int()
		Return GetRelativeDirection() = Building.Elevator.Direction And GetIntendedDirection() = Building.Elevator.Direction
	End Method		
	
	Method ToString:string()
'		local direction:int = 0
'		if (initialFrom < floornumber)
'			direction = 1
'		else
'			direction = -1
'		Endif
		...
		If call = 1
			Return "C   " + initialFrom + " -> *" + Building.Elevator.CurrentFloor + "* -> " + floornumber + " ( -> " + who.getFloor(who.target) + ")    R:" + GetRelativeDirection() + "/" + GetIntendedDirection() + "|" + CorrectDirection() + "   = " + who.name
		Else
			Return "S   " + initialFrom + " -> *" + Building.Elevator.CurrentFloor + "* -> " + floornumber + " ( -> " + who.getFloor(who.target) + ")    R:" + GetRelativeDirection() + "/" + GetIntendedDirection() + "|" + CorrectDirection() + "   = " + who.name + "      [" + Building.Elevator.CurrentFloor + " | " + initialFrom + " < " + floornumber + "]"
		Endif		
	End Method
	
	Method Compare:Int(otherObject:Object)	
		Local otherObj:TFloorRoute = TFloorRoute(otherObject)
		
		'Wenn das direkte UND ggf. indirekte Ziel in der richtigen Richtung liegt ist es besser (-1)
		If CorrectDirection()
			If Not otherObj.CorrectDirection() Then Return -1
		Else
			If otherObj.CorrectDirection() Then Return 1
		Endif
		
		'Wenn zumindest das direkte Ziel in der richtigen Richtung liegt ist es besser (-1)
		If GetRelativeDirection() = Building.Elevator.Direction
			If Not otherObj.GetRelativeDirection() = Building.Elevator.Direction Then Return -1
		Else
			If otherObj.GetRelativeDirection() = Building.Elevator.Direction Then Return 1
		Endif
		
		'Je geringer der Abstand desto besser: 		
		local diff1:int = GetDifference(floornumber, Building.Elevator.CurrentFloor)
		local diff2:int = GetDifference(otherObj.floornumber, Building.Elevator.CurrentFloor)
		If diff1 = diff2 Then Return 0				
		If diff1 < diff2 Then Return -1 Else Return 1
	End Method
	
	Method GetDifference:int(val1:int, val2:int)
		If val1 > val2
			Return val1 - val2
		Elseif val1 < val2
			Return val2 - val1
...
Type TElevator
	...
	'Field WaitAtFloorTime:Int				= 2000 		'Wie lange (Millisekunden) werden die Türen offen gelassen		
	...
	Method EnterTheElevator:int(figure:TFigures, myTargetFloor:int=-1) 'bzw. einsteigen				
		...
		If myTargetFloor <> -1 And Direction <> CalcDirection(CurrentFloor, myTargetFloor) Then
			print "abgelehnt: " + figure.name + " (" + myTargetFloor + "; " + CalcDirection(CurrentFloor, myTargetFloor)  + ")"
			Return False	
		Endif
		...
		If Not passengers.Contains(figure)
			...
			if (currentRoute <> null)
				print "Entferne Call-Route von: " + currentRoute.who.name
	...
	Method LeaveTheElevator(figure:TFigures) 'aussteigen
		...
		print "Entferne Send-Route von: " + figure.name
	...
	Method AddFloorRoute:Int(floornumber:Int, call:Int = 0, who:TFigures, fromNetwork:Int = False)
		If Not ElevatorCallIsDuplicate(floornumber, who) Then 'Prüfe auf Duplikate
			FloorRouteList.AddLast(TFloorRoute.Create(floornumber, call, who))
	...
	Method CalculateNextTarget:int()
		if TemporaryRouteList = null
			TemporaryRouteList = FloorRouteList.Copy()						
			TemporaryRouteList.Sort()
	...
	Method Update(deltaTime:Float=1.0)					
		'TODOX: Prüfen ob das irgendeinen Sinn macht!
...
'		Print "ElevatorStatus = " + ElevatorStatus + "        " + CurrentFloor  + " -> " + TargetFloor 
		...
		If ElevatorStatus = 1 or ElevatorStatus = 3 Then spriteDoor.Update(deltaTime) 'Türe animieren
